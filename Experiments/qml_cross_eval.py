...
def select_model(model_name, NUM_CLASSES, N_QUBITS=5, N_LAYERS=3, N_FEATURES=4, N_PACKETS=10, dev_name='default.qubit', N_SHOTS=1024):
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path of the directory where is the experiment (i.e., the trained model)"
    )
...
def main():
    ...
    with open(cfg_file, "w") as f:
        ...
        f.write(f"Source Dataset path = {source_dataset_path}\n")
        f.write(f"Target Dataset path = {target_dataset_path}\n")
    ...
    with open(source_dataset_path, "rb") as f:
        X_raw_src = np.array(pickle.load(f), dtype=np.float32)
        y_raw_src = np.array(pickle.load(f))

    print("Loading test dataset...")
    with open(target_dataset_path, "rb") as f:
        X_raw_tgt = np.array(pickle.load(f), dtype=np.float32)
        y_raw_tgt = np.array(pickle.load(f))

    le = LabelEncoder()
    y_encoded_src = le.fit_transform(y_raw_src)
    NUM_CLASSES = len(le.classes_)
    print(f"Number of classes: {NUM_CLASSES}")
    print("Classes:", le.classes_)
    ordinal_classes = np.unique(y_encoded_src)
    labels_map_file = f"{output_dir}/source_labels_map.json"
    dictionary = {}
    ...
    for (train_src, test_src), (_, test_tgt) in zip(kfold.split(X_raw_src, y_raw_src), kfold.split(X_raw_tgt, y_raw_tgt)):
        ...
        src_mask = y_raw_src[test_src] == 'benign'
        tgt_mask = y_raw_tgt[test_tgt] != 'benign'
        src_indices = test_src[src_mask]
        tgt_indices = test_tgt[tgt_mask]

        X_test_proc = np.concatenate([preprocess_data(X_raw_src[src_indices], scaler), preprocess_data(X_raw_tgt[tgt_indices], scaler)], axis=0)
        y_test = np.concatenate([
            np.full(len(src_indices), benign_idx),   # benign label for source
            np.full(len(tgt_indices), -1)              # -1 for target (unknown/attack)
        ], axis=0)
        ...
        all_labels_binary = np.where(np.array(all_labels) == benign_idx, 'benign', 'malicious')
        all_preds_binary  = np.where(np.array(all_preds)  == benign_idx, 'benign', 'malicious')
        report_dict = classification_report(all_labels_binary, all_preds_binary, digits=4, zero_division=0)
        print(report_dict)
